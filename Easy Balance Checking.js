...
function balance(book) {
  ...
  let balance = Number(cleanLines[0]);
  let report = `Original Balance: ${balance.toFixed(2)}\r\n`;
  let totalExpense = 0, count = 0;
  
  for(let i = 1; i < cleanLines.length; i++){
    let parts = cleanLines[i].split(' ');
    let number = parts[0], category = parts[1], amount = Number(parts[2]);
    balance -= amount;
    totalExpense += amount;
    count ++;
    report += `${number} ${category} ${amount.toFixed(2)} Balance ${balance.toFixed(2)}\r\n`;
  };
    
    report += `Total expense  ${totalExpense.toFixed(2)}\r\n`;
    report += `Average expense  ${(totalExpense / count).toFixed(2)}`;
    return report;
};