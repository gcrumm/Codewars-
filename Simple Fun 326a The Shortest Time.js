...
function shorterestTime(n,m,speeds) {
  let stairs = speeds[3] * n;
  let stairsAndElevator = Math.abs(n - m) * speeds[3] + m * speeds[0] + speeds[1] * 2 + speeds[2];     
  let wait = Math.abs(m - n) * speeds[0];
  let elevator = wait + n * speeds[0] + speeds[1] * 2 + speeds[2];
  if(n === 0) return 0;
  return Math.min(stairs, stairsAndElevator, elevator)
}